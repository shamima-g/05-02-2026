...
5. **Additional input — API spec:** If `generated-docs/specs/api-spec.yaml` exists (generated by design-api-agent earlier in the DESIGN phase), reference it when identifying screens and data flows.
...
When the manifest's `artifacts.wireframes.userProvided` points to a prototype source directory:
