...
/**
 * Determine if INTAKE is completed (both artifacts exist).
 * Even when past INTAKE phase, verify artifacts actually exist on disk
 * to avoid false positives from interrupted or skipped phases.
 */
function isIntakeComplete() {
  const { intakeAgentDone, brdReviewAgentDone } = getIntakeSubStepStatus();
  return intakeAgentDone && brdReviewAgentDone;
}
...
/**
 * Read the intake manifest to determine which DESIGN artifacts are expected.
...
function buildTodoList(state) {
  ...
  // If still in INTAKE, DESIGN, or SCOPE, no epic items to show yet
  if (['INTAKE', 'DESIGN', 'SCOPE'].includes(state.currentPhase) && !scopeComplete) {
    return items;
  }
